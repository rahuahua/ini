...
func (s *Section) mapTo(val reflect.Value, isStrict bool) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if isNormalStruct || isNormalStructPtr {
			...
			if sec, err := s.f.GetSection(rawName); err == nil {
				if err = sec.mapTo(field, isStrict); err != nil {
					return fmt.Errorf("error mapping field(%s): %v", rawName, err)
				}
				continue
			}
		} else if tpField.Anonymous && rawName == "" {
...
func (s *Section) reflectFrom(val reflect.Value) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if isNormalStruct || isNormalStructPtr {
			...
			// 继承，如果有ini tag，按照tag独立出一个section
			sec, err := s.f.GetSection(rawName)
			if err != nil {
				sec, _ = s.f.NewSection(rawName)
			}
			if err = sec.reflectFrom(field); err != nil {
				return fmt.Errorf("error reflecting field (%s): %v", rawName, err)
			}
			continue
		} else if tpField.Anonymous && rawName == "" {
			// 继承，如果没有ini tag，展开到上一层section
			if err := s.reflectFrom(field); err != nil {
				return fmt.Errorf("error reflecting field (%s): %v", s.Name(), err)
			}
			continue
		}
